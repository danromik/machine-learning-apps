...
>
  {#each chat.items as item, i (i)}
    ...
    {:else if item.kind === 'system'}
      <div class="self-center my-1 flex items-center gap-2 text-[10px] uppercase tracking-wider
                  text-[var(--color-muted)]">
        <span class="h-px w-6 bg-[var(--color-border)]"></span>
        {item.text}
        <span class="h-px w-6 bg-[var(--color-border)]"></span>
      </div>
  ...
  {#if chat.items.length === 0}
    ...
    >
      ...
      <p class="mb-2 text-[var(--color-heading)] font-medium">RL Coach</p>
      <p>
