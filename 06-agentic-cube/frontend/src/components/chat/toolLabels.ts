...
export function toolLabel(name: string, input: unknown): string {
  ...
  switch (name) {
    ...
    case 'get_recent_progress': {
      ...
      return count
        ? `Reading last ${count} ${pluralize(count, 'iteration')}`
        : 'Reading recent progress';
    }
    case 'get_run_status':
      return 'Checking the training run';
    ...
    case 'set_environment':
      return 'Configuring the cube + curriculum';
    case 'set_algorithm': {
    ...
    case 'set_curriculum_schedule':
      return 'Adjusting the curriculum';
    case 'set_curriculum_depth': {
      const k = n(args?.k);
      return k ? `Setting scramble depth k=${k}` : 'Setting scramble depth';
    }
    ...
    case 'train_n_iterations': {
      const count = n(args?.n);
      return count
        ? `Training (${count} ${pluralize(count, 'iteration')})`
        : 'Training';
    }
    case 'evaluate': {
      const count = n(args?.n);
      return count ? `Evaluating ${count} scrambles` : 'Evaluating solve-rate';
    }
    case 'watch_agent_play':
      return 'Attempting a scramble';
    case 'start_training_run':
      return 'Starting the background training run';
    case 'stop_training_run':
      return 'Stopping the training run';
    case 'update_training_report':
      return 'Updating the training report';
