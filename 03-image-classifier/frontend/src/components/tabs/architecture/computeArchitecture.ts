...
/** Generic CNN suggestion for a 96×96 RGB image classifier. Three
 *  conv blocks (channels 32 → 64 → 128) feeding a 256-unit FC head.
 *  Tuned for Imagenette's 10-class scale. */
export function suggestArchitecture(_opts: SuggestInput): ArchitectureSuggestion {
  const channels = [32, 64, 128];
  const fcWidth = 256;
