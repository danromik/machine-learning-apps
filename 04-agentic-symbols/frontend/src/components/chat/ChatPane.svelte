<!--
  ML Engineer chat pane. Composes the usage bar, session menu,
...
<script lang="ts">
  ...
  function applyEvent(ev: ChatEvent) {
    if (ev.type === 'text_delta') {
      ...
      // Final assistant text for one content block. Streaming should
      // have already filled the bubble — mark it closed. If for some
      // reason we missed the streaming (e.g. cache hit, no deltas),
      // create the bubble here.
      const last = chat.items[chat.items.length - 1];
      if (last && last.kind === 'text' && last.role === 'assistant' && last.streaming) {
        // Reconcile: trust the final text over accumulated deltas, in
        // case the stream dropped a frame.
      ...
      // Close any open assistant bubble.
      closeOpenBubble();
      chat.items.push({
        kind: 'tool',
        id: ev.id,
      ...
      // Only used when reconstructing past sessions.
      chat.items.push({
        kind: 'text',
        role: 'user',
        text: ev.text,
      ...
      if (typeof ev.input_tokens === 'number')
        chat.usage.input_tokens = ev.input_tokens;
      if (typeof ev.output_tokens === 'number')
        chat.usage.output_tokens = ev.output_tokens;
      ...
      if (typeof ev.total_cost_usd === 'number') {
        chat.usage.total_cost_usd += ev.total_cost_usd;
      }
      // Final usage may also live here (the stream might not have
      // emitted a usage event for short turns).
      const u = ev.usage as Record<string, number> | null;
      if (u) {
        if (typeof u.input_tokens === 'number')
          chat.usage.input_tokens = u.input_tokens;
        if (typeof u.output_tokens === 'number')
          chat.usage.output_tokens = u.output_tokens;
      }
...
>
  ...
  >
    <textarea
      ...
      oninput={onInput}
      onkeydown={onKeydown}
      placeholder="Ask the ML Engineer…"
      rows="1"
